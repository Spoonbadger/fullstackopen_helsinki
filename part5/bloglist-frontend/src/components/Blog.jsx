    ...
    <div>
      ...
      <div style={showWhenInfoVisible}>
        <div className='expandedBlogView' style={blogStyle}>
          ...
          <div data-testid="blog-likes">likes: {blog.likes}
            <button data-testid='like-button' onClick={handleLike} style={buttonStyle}>like</button>
          </div>
          <div className='blog-author'>{blog.author}</div>
          {blogPoster && <button style={buttonStyle} onClick={() => deleteBlog(blog)}>delete</button>}
        </div>
      </div>
    </div>
  )
