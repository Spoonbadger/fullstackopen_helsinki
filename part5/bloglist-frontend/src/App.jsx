...
const App = () => {
  ...
  const updateTheLikeCount = async (updateBlog) => {
    ...
    setBlogs((blogs) => {
      const updatedBlogs = blogs.map(blog => blog.id === updatedBlog.id ? updateBlog : blog)
  ...
  return (
    <div>
      <h2>blogs</h2>
      <Notification message={ notification?.message } type={ notification?.type}/>
      <p><span>{user.name} logged in</span><button data-testid="logout-button" onClick={handleLogout}>logout</button></p>
      <div>
        {blogForm()}
      </div>
      {blogs.map(blog =>
